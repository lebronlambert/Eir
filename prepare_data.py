...
def getGeneDisease2Pmid(path):
  ...
  return gd2pmid, pmid2title

if not (os.path.isfile('input/gd2pmid.npy') and os.path.isfile('input/pmid2title.npy')):
  print '(GENE, disease) -> PMID...'
  path = 'raw/final_output'
  gd2pmid, pmid2title = getGeneDisease2Pmid(path)
  np.save('input/gd2pmid.npy',gd2pmid)
  np.save('input/pmid2title.npy',pmid2title)
...
def replaceOntology(pmid2sct, corpus, g2c, d2c):
  ...
  for gdp in corpus:
    ...
    samples.append(gdscts)
    samples_pmid.append(pmid)
    samples_gene.append(gene)
    samples_disease.append(disease)
  num_sample = len(corpus)
  print 100.0*len(samples)/num_sample
  return samples, samples_pmid, samples_gene, samples_disease
...
if not (os.path.isfile('input/pos_samples') \
        and os.path.isfile('input/neg_samples') ):
  print 'Replacing ontologies...'
  pos_samples, pos_samples_pmid, pos_samples_gene, pos_samples_disease = replaceOntology(pmid2sct, pos_corpus[0:170665], g2c, d2c)
  neg_samples, neg_samples_pmid, neg_samples_gene, neg_samples_disease = replaceOntology(pmid2sct, neg_corpus[0:170665], g2c, d2c)
  import json
  with open('input/pos_samples', 'w') as f:
    json.dump(pos_samples,f)
  with open('input/neg_samples', 'w') as f:
  ...
  with open('input/neg_samples_pmid', 'w') as f:
    json.dump(neg_samples_pmid,f)
  with open('input/pos_samples_gene', 'w') as f:
    json.dump(pos_samples_gene,f)
  with open('input/neg_samples_gene', 'w') as f:
    json.dump(neg_samples_gene,f)
  with open('input/pos_samples_disease', 'w') as f:
    json.dump(pos_samples_disease,f)
  with open('input/neg_samples_disease', 'w') as f:
    json.dump(neg_samples_disease,f)
